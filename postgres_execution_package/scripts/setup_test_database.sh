...
# الترحيل (000 حتى 035) بالترتيب الرقمي من الصفر. الحزمة مكتفية ذاتيًا
...
REQUIRED_FILES=(
  ...
  "033_ana_events.sql"
  "034_cnt_articles_cms_seo.sql"
  "035_login_security_history.sql"
)

echo "=== التحقق من اكتمال جميع ملفات الترحيل المطلوبة (000-035) قبل أي تنفيذ ==="
missing=0
...
echo "=== نجاح: جميع ملفات الترحيل الـ35 (000-035، بلا 024) موجودة فعليًا في الحزمة — Batch 2 Unit 1 ==="
...
echo "=== تطبيق كل ملفات الترحيل بالترتيب الرقمي (000 حتى 035) ==="
for f in "${REQUIRED_FILES[@]}"; do
  echo "--- تطبيق: $f ---"
...
echo "=== نجاح: قاعدة الاختبار '$DB_NAME' جاهزة بكل الجداول من الصفر (35 ملف ترحيل مُطبَّق) ==="
