...
class TestCR017PublicReadConsistency:
    """CR-017: قائمة المقالات وتفاصيل المقال المنشور عامتان بالكامل؛ غير
    المنشور يبقى بلا تغيير عن سلوكه السابق (يتطلب أي جلسة صالحة)."""
    ...
    def test_published_article_readable_without_session(self, app_and_client):
        ...
        article_id = client.post("/api/v1/content/articles", json={"title": "خبر", "body": "نص"}).json()["id"]
        client.post(f"/api/v1/content/articles/{article_id}/publish")
        client.post("/api/v1/auth/logout")
    ...
    def test_unpublished_article_still_requires_session_unchanged(self, app_and_client):
        app, client = app_and_client
        _login_as(app, client, "editor8@example.com", role="news_editor")
        article_id = client.post("/api/v1/content/articles", json={"title": "مسودة", "body": "نص"}).json()["id"]
        client.post("/api/v1/auth/logout")

        resp = client.get(f"/api/v1/content/articles/{article_id}")
        assert resp.status_code == 401

    def test_unpublished_article_still_readable_with_any_session_unchanged(self, app_and_client):
        """يحافظ على السلوك القديم حرفيًا: أي جلسة (بلا فحص دور) ترى أي حالة."""
        app, client = app_and_client
