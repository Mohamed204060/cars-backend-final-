...
class PctRepository(ABC):
    ...
    @abstractmethod
    def list_parts(self, status: str, q: Optional[str], page: int,
                    page_size: int) -> "tuple[List[CatalogPart], int]":
        """CR-015: status إلزامي دائمًا هنا (لا وضع 'كل الحالات') — القيمة
        الافتراضية 'approved' تُحسَم في طبقة الـAPI، والتحقق من صلاحية طلب
        'proposed' يتم هناك أيضًا، لا في هذه الطبقة."""
        raise NotImplementedError
...
class PostgresPctRepository(PctRepository):
    ...
    def list_parts(self, status: str, q: Optional[str], page: int, page_size: int):
        # يعتمد على idx_catalog_parts_status، وidx_pct_localized_names_value عند وجود q
        offset = (page - 1) * page_size
        filters = ["cp.status = %(status)s"]
        params = {"status": status, "limit": page_size, "offset": offset}
        join_clause = ""
        if q is not None:
            join_clause = "JOIN pct.localized_names ln ON ln.catalog_part_id = cp.id"
            filters.append("ln.name_value ILIKE %(q)s")
            params["q"] = f"%{q}%"
        where_clause = " AND ".join(filters)
        with self._connection.cursor() as cur:
            cur.execute(
                f"SELECT COUNT(DISTINCT cp.id) AS total FROM pct.catalog_parts cp {join_clause} WHERE {where_clause}",
                params,
            )
            total = cur.fetchone()["total"]
            cur.execute(
                f"SELECT DISTINCT cp.id, cp.category_id, cp.status, cp.created_at "
                f"FROM pct.catalog_parts cp {join_clause} WHERE {where_clause} "
                f"ORDER BY cp.created_at DESC LIMIT %(limit)s OFFSET %(offset)s",
                params,
            )
            rows = cur.fetchall()
        return [CatalogPart(id=r["id"], category_id=r["category_id"], status=r["status"]) for r in rows], total
...
class InMemoryPctRepository(PctRepository):
    ...
    def list_parts(self, status: str, q: Optional[str], page: int, page_size: int):
        items = [p for p in reversed(list(self._parts.values())) if p.status == status]
        if q is not None:
            matching_part_ids = {n.catalog_part_id for n in self._names if q.lower() in n.name_value.lower()}
            items = [p for p in items if p.id in matching_part_ids]
        total = len(items)
