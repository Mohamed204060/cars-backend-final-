...
class StoreRepository(ABC):
    ...
    @abstractmethod
    def get_store_by_owner_id(self, owner_user_ref_id: str) -> Optional[Store]:
        """امتداد Store+Inventory Contract Extension: يدعم اشتقاق store_id من
        جلسة المستخدم مباشرة (متجر واحد لكل بائع، REQ-STR-001)، بدلًا من
        مطالبة العميل بإرساله صراحةً — مطابقةً للعقد /inventory-items المعتمَد سلفًا."""
        raise NotImplementedError

    @abstractmethod
    def get_store_by_owner_id(self, owner_user_ref_id: str) -> Optional[Store]:
        """امتداد Store+Inventory Contract Extension: يدعم اشتقاق store_id
        من جلسة المستخدم (متجر واحد لكل بائع، REQ-STR-001)، بدلًا من قبول
        store_id صراحة في طلب إنشاء عنصر المخزون (العقد المعتمَد أصلاً)."""
        raise NotImplementedError
    ...
    @abstractmethod
    def list_stores(self, status: Optional[str], owner_user_ref_id: Optional[str],
                     page: int, page_size: int) -> "tuple[list, int]":
        """CR-015: استخدام إداري أساسًا — لا Directory عام مفعَّل (راجع CR-015 §0)."""
        raise NotImplementedError
...
class PostgresStoreRepository(StoreRepository):
    ...
    def get_store_by_owner_id(self, owner_user_ref_id: str) -> Optional[Store]:
        # يعتمد على idx_stores_owner؛ يفترض متجرًا واحدًا لكل بائع (REQ-STR-001، لا فروع)
        query = ("SELECT id, owner_user_ref_id, status, country_ref_id, city_ref_id FROM str.stores "
                 "WHERE owner_user_ref_id = %(owner_id)s LIMIT 1")
        with self._connection.cursor() as cur:
            cur.execute(query, {"owner_id": owner_user_ref_id})
            row = cur.fetchone()
        if row is None:
            return None
        return Store(id=row["id"], owner_user_ref_id=row["owner_user_ref_id"], status=row["status"],
    ...
    def get_store_by_owner_id(self, owner_user_ref_id: str) -> Optional[Store]:
        # يعتمد على idx_stores_owner؛ متجر واحد لكل بائع (REQ-STR-001)،
        # فأي تعدد غير متوقَّع يُعامَل بأخذ أول صف فقط دون خطأ
        query = ("SELECT id, owner_user_ref_id, status, country_ref_id, city_ref_id FROM str.stores "
                 "WHERE owner_user_ref_id = %(owner_user_ref_id)s LIMIT 1")
        with self._connection.cursor() as cur:
            cur.execute(query, {"owner_user_ref_id": owner_user_ref_id})
            row = cur.fetchone()
        if row is None:
            return None
        return Store(id=row["id"], owner_user_ref_id=row["owner_user_ref_id"], status=row["status"],
    ...
    def list_stores(self, status: Optional[str], owner_user_ref_id: Optional[str], page: int, page_size: int):
        ...
        if owner_user_ref_id is not None:
            filters.append("owner_user_ref_id = %(owner_id)s")
            params["owner_id"] = owner_user_ref_id
        where_clause = f"WHERE {' AND '.join(filters)}" if filters else ""
        with self._connection.cursor() as cur:
            cur.execute(f"SELECT COUNT(*) AS total FROM str.stores {where_clause}", params)
            total = cur.fetchone()["total"]
            cur.execute(
                f"SELECT id, owner_user_ref_id, status, country_ref_id, city_ref_id FROM str.stores "
                f"{where_clause} ORDER BY created_at DESC LIMIT %(limit)s OFFSET %(offset)s",
                params,
            )
            rows = cur.fetchall()
        items = [Store(id=r["id"], owner_user_ref_id=r["owner_user_ref_id"], status=r["status"],
                        country_ref_id=r["country_ref_id"], city_ref_id=r["city_ref_id"]) for r in rows]
        return items, total
...
class InMemoryStoreRepository(StoreRepository):
    ...
    def get_store_by_owner_id(self, owner_user_ref_id: str) -> Optional[Store]:
        for store in self._stores.values():
            if store.owner_user_ref_id == owner_user_ref_id:
                return store
        return None
    ...
    def list_stores(self, status: Optional[str], owner_user_ref_id: Optional[str], page: int, page_size: int):
        items = list(reversed(list(self._stores.values())))
        if status is not None:
            items = [s for s in items if s.status == status]
        if owner_user_ref_id is not None:
            items = [s for s in items if s.owner_user_ref_id == owner_user_ref_id]
        total = len(items)
