...
class PurchaseRequestDisplayView:
    ...
    Batch 1 (Approved VCT Design Baseline): generation_name/trim_name/
    model_year أصبحت متاحة الآن فعليًا (كانت مؤجَّلة صراحةً بانتظار اكتمال
    VCT — هذه الدفعة هي ذلك الاكتمال). condition_code/notes تُضاف أيضًا
    (موجودة كأعمدة فعلية منذ CR-022، لم تكن معروضة في هذا الـProjection
    تحديدًا). كلها Optional/None-safe للسجلات القديمة (بلا توطين، بلا سنة،
    بلا حالة محدَّدة).
    """
    id: str
...
class PurchaseRequest:
    ...
    # Batch 1 §23: سنة موديل محدَّدة اختياريًا ضمن trim_ref_id (يجب أن تتبع نفس trim_ref_id)
    trim_model_year_ref_id: Optional[str] = None
...
class TrimNotValidForPurchaseRequestError(Exception):
    """Batch 1 (Approved VCT Design Baseline §23): trim_ref_id غير موجود في VCT الحقيقي."""


class TrimModelYearNotValidForPurchaseRequestError(Exception):
    """Batch 1 §23: trim_model_year_ref_id غير موجود، أو لا ينتمي لنفس trim_ref_id المحدَّد."""


class InvalidTrimError(Exception):
    """Batch 1 §23: trim_ref_id غير موجود في VCT — لا نص حر غير موثوق بعد الآن."""


class InvalidTrimModelYearError(Exception):
    """Batch 1 §23: trim_model_year_ref_id غير موجود، أو لا يتبع trim_ref_id المحدَّد."""
...
def create_purchase_request(buyer_user_ref_id: str, catalog_part_ref_id: str, trim_ref_id: str,
                             ...
                             notes: Optional[str] = None, is_condition_ref_valid_checker=None,
                             is_trim_valid_checker=None, trim_model_year_ref_id: Optional[str] = None,
                             is_trim_model_year_valid_for_trim_checker=None) -> PurchaseRequest:
    """
    ...
    (نقاط رمز Unicode). لا Backfill، لا صور، لا CR-020 Search.

    Batch 1 (Approved VCT Design Baseline §23): trim_ref_id يُتحقَّق منه
    فعليًا عبر VCT الحقيقي الآن (لا نص حر غير موثوق كما قبل هذه الدفعة).
    trim_model_year_ref_id اختياري (سنة دقيقة ضمن نفس trim_ref_id)؛ يُتحقَّق
    أنه ينتمي فعليًا لنفس trim_ref_id المحدَّد، لا أي سنة عشوائية.
    """
    if is_trim_valid_checker is not None and not is_trim_valid_checker(trim_ref_id):
        raise TrimNotValidForPurchaseRequestError(
            f"فئة السيارة المرجعية '{trim_ref_id}' غير موجودة أو غير صالحة."
        )

    if trim_model_year_ref_id is not None and is_trim_model_year_valid_for_trim_checker is not None \
            and not is_trim_model_year_valid_for_trim_checker(trim_model_year_ref_id, trim_ref_id):
        raise TrimModelYearNotValidForPurchaseRequestError(
            f"سنة الموديل المرجعية '{trim_model_year_ref_id}' غير موجودة أو لا تنتمي لفئة السيارة '{trim_ref_id}'."
        )
    ...
    return PurchaseRequest(id="", buyer_user_ref_id=buyer_user_ref_id,
                            ...
                            trim_model_year_ref_id=trim_model_year_ref_id)
...
def create_purchase_request_via_repository(repository, buyer_user_ref_id: str,
                                            ...
                                            condition_ref_id: Optional[str] = None,
                                            notes: Optional[str] = None,
                                            is_condition_ref_valid_checker=None,
                                            is_trim_valid_checker=None,
                                            trim_model_year_ref_id: Optional[str] = None,
                                            is_trim_model_year_valid_for_trim_checker=None) -> PurchaseRequest:
    pr = create_purchase_request(buyer_user_ref_id, catalog_part_ref_id, trim_ref_id,
                                  ...
                                  is_condition_ref_valid_checker, is_trim_valid_checker,
                                  trim_model_year_ref_id, is_trim_model_year_valid_for_trim_checker)
...
def get_purchase_request_display_via_repository(repository, pr_id: str):
    """Batch 1 (Offers Integration): طبقة تنسيق رقيقة، بنفس مبدأ الدالة أعلاه."""
