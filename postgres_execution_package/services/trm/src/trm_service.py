...
def update_rating_via_repository(repository, rating_id: str, new_score: int, new_comment: Optional[str] = None) -> Rating:
    """امتداد Final Backend Batch Contract Extension: كان الغلاف مفقودًا رغم وجود update_rating_score_and_comment."""
    rating = repository.get_rating_by_id(rating_id)
    if rating is None:
        raise ValueError(f"لا يوجد تقييم بالمعرّف: {rating_id}")
    update_rating_score_and_comment(rating, new_score, new_comment)
    return repository.update_rating(rating)
