...
# تصحيح أمني (منع تسريب معلومات داخلية) — نفس نمط auth_api.py._logger؛
# تفاصيل استثناءات قاعدة البيانات الحقيقية تُسجَّل من جانب الخادم فقط.
...
def admin_list_conversation_messages(
    ...
    try:
        ...
        # تصحيح أمني: التفاصيل الخام (قد تكشف أسماء أعمدة/جداول/شذرات SQL
        # عند فشل حقيقي في PostgreSQL) تُسجَّل من جانب الخادم فقط، لا تدخل
        # استجابة HTTP العامة للعميل أبدًا.
        _logger.error(
            "فشل تسجيل دليل التدقيق الإلزامي للوصول الحسّاس (conversation_id=%s, correlation_id=%s): %s",
            conversation_id, correlation_id, exc, exc_info=True,
        )
        raise error(
            correlation_id, status.HTTP_503_SERVICE_UNAVAILABLE, "MESSAGE_ACCESS_AUDIT_PERSISTENCE_FAILED",
            "تعذّر تسجيل دليل التدقيق الإلزامي لهذا الوصول الحسّاس؛ لن يُعاد أي محتوى بلا دليل مُثبَّت. يرجى إعادة المحاولة لاحقًا.",
        )
